...
use prunifier::{ConfigLoader, PrunifierConfig, PrunifierError};

/// Helper: write YAML content to a unique temp file and return its path.
fn write_temp_yaml(content: &str) -> PathBuf {
    let dir = std::env::temp_dir().join("prunifier_test_config_loader_it");
...
/// Loading a non-existent path should succeed and return PrunifierConfig::default().
#[test]
fn test_missing_config_uses_defaults() {
    let non_existent = Path::new("/tmp/does_not_exist_prunifier_loader_test.yaml");
    let config =
